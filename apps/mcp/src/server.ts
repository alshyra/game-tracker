import { type CoreServices, type Game, STATUSES, type Status } from "@gameshelf/core";
...
function resolveGame(games: Game[], query: string): Resolution {
  ...
  if (partial.length === 1 && partial[0]) return { key: partial[0].key, title: partial[0].title };
...
/**
 * Construit le serveur MCP Gameshelf avec ses outils. Indépendant du transport :
 * utilisé en stdio (CLI/agent local) comme en Streamable HTTP (agent distant).
 */
export function createGameshelfMcpServer(services: CoreServices): McpServer {
